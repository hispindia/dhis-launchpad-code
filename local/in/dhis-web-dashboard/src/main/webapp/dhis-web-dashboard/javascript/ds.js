...
// LineListing Data Status Related Functions
function getOUDeatilsForLineListingDataStatus( orgUnitIds )
{
	jQuery.postJSON("getOrgUnitName.action",{
  	  id : orgUnitIds[0]
   }, function( json ){
	   ...
	   document.getElementById("orgUntId").value = orgUnitId;
        
   });
}

function generateLineListingDataStatus()
{
    if(formValidationsForLineListingDataStatus())
    {
    	var url = "dataStatusLineListingResult.action?";
    	...
    	{
    		selectedDataSets : getFieldValue( 'selectedDataSets' ),
    		orgUntId : getFieldValue( 'orgUntId' ),
    		startPeriodId : getFieldValue( 'startPeriodId' ),
    		endPeriodId : getFieldValue( 'endPeriodId' ),
    		
    	} ).dialog( {
    		title: 'LineListing Data Entry Status Result',
    		maximize: true, 
    		closable: true,
    		modal:true,
    		overlay:{ background:'#000000', opacity:0.1 },
...
//LineListing DataStatus Form Validations
function formValidationsForLineListingDataStatus()
{
    var dataSetList = document.getElementById("selectedDataSets");
    var dataSetListIndex = dataSetList.selectedIndex;
    
    var orgunitName = document.getElementById("ouNameTB").value;
    
    var startPeriodObj = document.getElementById('startPeriodId');
    var endPeriodObj = document.getElementById('endPeriodId');
    ...
    if( dataSetListIndex < 0 || dataSetList.options[dataSetListIndex].text == null ) 
    {
        alert("Please Select DataSet"); 
        return false;
    }
    
    else if( orgunitName == null || orgunitName == "" || orgunitName == " " ) 
    {
...
// LineListing formValidations Function End	
