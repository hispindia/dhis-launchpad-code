	...
	else
	{
		...
		jQuery('#AddASHABeneficiaryFormDiv').load('loadASHABeneficiaryAddForm.action',
			...
			}, function()
			{
				...
				loadASHAActivityDetailsForm( ashaId, selectedPeriodId );
...
// ASHA Activity Details Form
function loadASHAActivityDetailsForm( ashaId,selectedPeriodId )
{
	
	$( '#ASHAActivityDetailsDiv' ).html('');
	...
	{
		document.getElementById('overlay').style.visibility = 'visible';
		jQuery('#ASHAActivityDetailsDiv').load('loadASHAActivityDetailsForm.action',
			{
				id:ashaId,
				selectedPeriodId:selectedPeriodId
			}, function()
			{
				showById('ASHAActivityDetailsDiv');
				document.getElementById('overlay').style.visibility = 'hidden';
...
function saveASHAActivityDetailsDataEntryForm()
{
	var selectedPeriodId = $( '#selectedPeriodId' ).val();
	...
	$.ajax({
      type: "POST",
      url: 'saveASHAActivityDetails.action',
      data: getASHAActivityDetailsParamsForDiv('ASHAActivityDetailsDataEntryForm'),
...
function loadASHABeneficiaryAndActivityAmountDetailsForm( selectedPeriodId, organisationUnitId, ashaId )
{
	...
	jQuery('#addUpdateASHAProfileDiv').load('loadASHABeneficiaryAndActivityAmountDetailsForm.action',
		{
			id:ashaId,
			selectedPeriodId:selectedPeriodId,
...
function getASHAActivityDetailsParamsForDiv( formDataDiv )
{
	var params = '';
	var dateOperator = '';
	...
	else
	{
		...
		$.ajax({
	      ...
	      success: function( json ) {
			...
			showAddASHABeneficiaryForm( ashaId, selectedPeriodId );
