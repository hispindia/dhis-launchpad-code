...
public class GetClosingBalancAction implements Action 
{
    ...
    */
    ...
    public String execute() 
    {
         ...
         OrganisationUnit organisationUnit = organisationUnitService.getOrganisationUnit( Integer.parseInt(organisationUnitId.trim()));
        
         String storedBy = currentUserService.getCurrentUsername();
            
         //DataElement dataElement = dataElementService.getDataElement( Integer.parseInt(dataElementId[0].trim()) );
         //DataElementCategoryOptionCombo optionCombo = categoryService.getDataElementCategoryOptionCombo( Integer.parseInt(dataElementId[1].trim()) );
       ...
       /*  
        if ( dataValue == null )
        {
            ...
            {                   
                dataValue = new DataValue( dataElement, period, organisationUnit, closingBalance, storedBy, now, null, optionCombo );
                dataValueService.addDataValue( dataValue );                
            }
         }
         else
         {
             dataValue.setValue( closingBalance );
            ...
            String[] firstperiod=calulatePeriod[1].split("-");  
                ...
                if(Integer.parseInt(firstperiod[1])> 1 || Integer.parseInt(firstperiod[1])< 12)
                {
                    if((Integer.parseInt(firstperiod[2])==1 || Integer.parseInt(firstperiod[2])< 15))
                    {
                        ...
                        nextPeriod= calulatePeriod[0]+"_"+firstperiod[0]+"-"+firstperiod[1]+"-"+date+"_"+firstperiod[0]+"-"+firstperiod[1]+"-"+monthDays[Integer.parseInt(firstperiod[1])-1];
                    }
                    else if(Integer.parseInt(firstperiod[2])>=15)
                    {               
                        int month=Integer.parseInt( firstperiod[1] )+1;                                                 
                        nextPeriod= calulatePeriod[0]+"_"+firstperiod[0]+"-"+month+"-01_"+firstperiod[0]+"-"+month+"-15";
                    }                                                
                }                               
        ...
        {               
            String[] splitperiod=calulatePeriod[1].split("-");
            if(Integer.parseInt(splitperiod[2])<= Integer.parseInt(monthDays[Integer.parseInt(splitperiod[1])-1]+"")-7)
            {
                int day=Integer.parseInt( splitperiod[2] )+7;
                nextPeriod= calulatePeriod[0]+"_"+splitperiod[0]+"-"+splitperiod[1]+"-"+day; 
            }
            else if(Integer.parseInt(splitperiod[2])> Integer.parseInt(monthDays[Integer.parseInt(splitperiod[1])]+"")-7)
            {
                int month=Integer.parseInt( splitperiod[1] )+1;
        ...
        /*
        DataElement nextdataElement = dataElementService.getDataElement( Integer.parseInt(nextdataElementId[0].trim()) );
        DataElementCategoryOptionCombo nextoptionCombo = categoryService.getDataElementCategoryOptionCombo( Integer.parseInt(nextdataElementId[1].trim()) );
        */
        ...
        DataValue nextdataValue = dataValueService.getDataValue( organisationUnit, nextdataElement, nextperiodId, nextoptionCombo );

        if ( nextdataValue == null )
        {                       
            nextdataValue = new DataValue( nextdataElement, nextperiodId, organisationUnit, closingBalance, storedBy, now, null, nextoptionCombo );
