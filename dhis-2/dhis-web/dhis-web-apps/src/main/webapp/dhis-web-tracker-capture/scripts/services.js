...
var trackerCaptureServices = angular.module('trackerCaptureServices', ['ngResource'])
    ...
    .factory('OrgUnitFactory', function($http,$q) {
        ...
        return {
            ...
            getOu : function(ou){
                var def = $q.defer();
                $http.get(  '../api/organisationUnits/' + ou.id +".json?fields=id,name,parent[id]"  ).then(function(response){

                    def.resolve(response.data);
                });
                return def.promise;
            },
            getCurrentToRootName : function(ou,result,def) {
                var promise = this.getOu(ou);
                var thiz = this;
                promise.then(function (ou) {

                    if (ou.parent ==  undefined){
                        result =  ou.name.substring(0,2)+result;
                        def.resolve(result);
                        return;
                    }else{
                        result =  ou.name.substring(0,2)+result;
                        return thiz.getCurrentToRootName(ou.parent,result,def);
                    }
                });
                return def.promise();
    ...
    .service('RegistrationService', function(DialogService, TEIService, $q){
        return {
            ...
            processForm: function(existingTei, formTei, attributesById,totalTei){
                var BeneficiaryId="";
                var showAlert = true;
                ...
                for(var k in attributesById){
                    if (attributesById[k].name == "Beneficiary Id"){ // Case for generating system ID; Not affecting formEmpty flag.
                        if (existingTei[k] !=null){
                            formTei[k] = existingTei[k];
                            showAlert=false;
                        }else{
                            formTei[k] = totalTei;
                        }
                    ...
                    if( formTei[k] ){
                        if (attributesById[k].name == "Beneficiary Id") {
                            var val = totalTei.toUpperCase();
                            BeneficiaryId = val;
                            var att = attributesById[k];
                            tei.attributes.push({attribute: att.id, value: formTei[k], displayName: att.name, type: att.valueType});
                            if (showAlert)
                            alert("Beneficiary Registered with Id : "+BeneficiaryId);
                        }else{
                            var att = attributesById[k];
                            tei.attributes.push({attribute: att.id, value: formTei[k], displayName: att.name, type: att.valueType});
                            formEmpty = false;
    ...
    .factory('TEIService', function($http, $q, AttributesFactory) {
        ...
        return {
            ...
            getTotalTei: function(ouId){
                var def = $q.defer();

                var url =  '../api/trackedEntityInstances.json?ou=' + ouId + '&skipPaging=true';
                var promise = $http.get( url ).then(function(response){
                    def.resolve(response.data.height);
                });
                return def.promise;
            }
        };
    ...
    .factory('AttributesFactory', function($q, $rootScope, TCStorageService, orderByFilter, DateUtils, OptionSetService,$http) {
        ...
        return {
            ...
            formatAttributeValue: function(att, attsById, optionSets, destination){
             ...
             getAttributesByGroupName : function(name){
                     var promise = $http.get( '../api/trackedEntityAttributeGroups.json?filter=name:eq:'+name+'&fields=id,name,trackedEntityAttributes').then(function(response){
                         return response.data.trackedEntityAttributeGroups[0].trackedEntityAttributes;
                     });
                 return promise;
            }
...
.factory('programCriteriaFactory',function($http){
        return {
            getProgramsByCriteria: function (teas) {

                var commaSeparatedAttributesList = "";
                var commaSeparatedAttributesValueList = "";
                angular.forEach(teas, function(tea) {
                    if (commaSeparatedAttributesList.length <1) {
                        commaSeparatedAttributesList = tea.id;
                    } else {
                        commaSeparatedAttributesList = commaSeparatedAttributesList + "," + tea.id;
                    }

                    if (commaSeparatedAttributesValueList.length<1) {
                        commaSeparatedAttributesValueList = tea.value;
                    } else {
                        commaSeparatedAttributesValueList = commaSeparatedAttributesValueList + "," + tea.value;
                    }


                });
                    var promise = $http.get('../api/programCriterias.json?teas='+commaSeparatedAttributesList+'&ops=op1&values='+commaSeparatedAttributesValueList).then(function (response) {
    ...
    .service('EventUtils', function(DateUtils, PeriodService, CalendarService, OptionSetService, $filter, orderByFilter){
        ...
        var getEventDueDate = function(eventsByStage, programStage, enrollment,nextProgramStage){
            ...
            if(programStage.repeatable){
                ...
                if(evs.length > 0){
                    ...
                    if(programStage.periodType){
                        ...
                        if (nextProgramStage.sortOrder < 2)
                            offset = nextProgramStage.standardInterval;
                        else{
        ...
        return {
            createDummyEvent: function(eventsPerStage, tei, program,programStage, orgUnit, enrollment,nextProgramStage){
                if (!nextProgramStage){
                    nextProgramStage = programStage;
                }
                ...
                if(programStage.periodType){
                    ...
                    dummyEvent.dueDate = getEventDueDate(eventsPerStage, programStage, enrollment,nextProgramStage);
                }
