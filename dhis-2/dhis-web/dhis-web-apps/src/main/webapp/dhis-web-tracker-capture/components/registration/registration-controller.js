...
trackerCapture.controller('RegistrationController',
    function($rootScope,
        ...
        $scope.generateId = function (){
            ...
            defOu = $.Deferred();
            def = $.Deferred();
            promise = OrgUnitFactory.getCurrentToRootName($scope.selectedOrgUnit, "",defOu);
            promise.then(function(result){
                var promiseTeiTotal =TEIService.getTotalTei($scope.selectedOrgUnit.id);
                promiseTeiTotal.then(function(totalTei){
                    totalTei=totalTei+1;
                    var prefix = "";

                    if( totalTei <10) prefix="000";
                    else if (totalTei >9 && totalTei<100) prefix="00";
                    else if(totalTei>99 && totalTei<1000) prefix="0";
                    // change in requirement - adding random number
                    prefix=Math.floor(Math.random()*(9999-1000) + 1000);
                    def.resolve(result+prefix);
        ...
        $scope.registerEntity = function(destination){
            var BeneficiaryId="";
            var promise =this.generateId();
            promise.then(function(totalTei) {
                ...
                var result = RegistrationService.processForm($scope.tei, $scope.selectedTei, $scope.attributesById,totalTei);
