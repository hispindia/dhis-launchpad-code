...
trackerCapture.controller('DashboardController',
        ...
        DashboardLayoutService.get().then(function(response){
            ...
            if ($scope.selectedProgram != null) {
                angular.forEach($scope.programs, function (pr) {
                    if (pr.id === $scope.selectedProgram.id) {
        ...
        OptionSetService.getAll().then(function(optionSets){            
            ...
            AttributesFactory.getAll().then(function(atts){
                ...
                TEIService.get($scope.selectedTeiId, $scope.optionSets, $scope.attributesById).then(function(response){
                    ...
                    AttributesFactory.getAttributesByGroupName('Criteria').then(function(teas){
                        angular.forEach($scope.selectedTei.attributes, function(teaWithValue){
                            angular.forEach(teas,function(tea){
                                if (tea.id == teaWithValue.attribute){
                                    tea.value = teaWithValue.value;
                                }
                            })
                        })

                        programCriteriaFactory.getProgramsByCriteria(teas).then(function(programs){
                            $scope.filteredPrograms = programs;
                        });
	...
	$scope.transferTeOu = function(){

		var modalInstanceTeOu = $modal.open({
			templateUrl: 'components/utility/transferTeOu.html',
			controller: 'transferTeOuController',
	...
	modalInstanceTeOu.result.then(function (response) {
	$timeout(function() {
                       $scope.back();
                    },500)
    }, function () {
