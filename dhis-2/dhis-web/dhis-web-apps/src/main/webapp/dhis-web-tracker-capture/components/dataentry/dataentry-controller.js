...
trackerCapture.controller('DataEntryController',
    function($scope,
        ...
        $scope.showCreateEvent = function(stage,nextStage){
            if (nextStage == null){
                nextStage = stage;
            }
            var dummyEvent = EventUtils.createDummyEvent($scope.eventsByStage[nextStage.id],$scope.selectedEntity, $scope.selectedProgram,stage,  $scope.selectedOrgUnit, $scope.selectedEnrollment,nextStage);
            ...
            ModalService.showModal({}, modalOptions).then(function(result){
                ...
                DHIS2EventFactory.update(dhis2Event).then(function(data){
                    ...
                    if($scope.currentEvent.status === 'COMPLETED'){
                        ...
                        else  if($scope.currentEvent.status === 'COMPLETED' && $scope.currentStage.allowGenerateNextVisit){
                            if ($scope.currentEvent.krCkrYWQeR1== "Application approved" || $scope.currentStage.sortOrder > 1){
                                var programStagesSorted = $scope.programStages;
                                programStagesSorted= orderByFilter(programStagesSorted, '-sortOrder').reverse();
                                $scope.showCreateEvent(programStagesSorted[$scope.currentStage.sortOrder ],$scope.currentStage);
                            }else{$scope.showCreateEvent($scope.currentStage,null);}
                        }else{
        ...
        $scope.save = function () {
            ...
            DHIS2EventFactory.create(newEvents).then(function(data){
                if (data.importSummaries[0].status === 'ERROR') {
                    ...
                    newEvent= processEvent(newEvent,$scope.stagesById[newEvent.programStage]);
