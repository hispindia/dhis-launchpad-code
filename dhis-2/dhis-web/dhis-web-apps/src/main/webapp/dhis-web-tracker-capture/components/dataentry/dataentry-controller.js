...
trackerCapture.controller('DataEntryController',
        ...
        $scope.saveDatavalue = function (prStDe) {
            ...
            if ($scope.currentEventOriginal[prStDe.dataElement.id] !== value) {
                ...
                if (value) {
                    ...
                    //get tracked entity instance ;change value of working status attribute and save.
                    if( prStDe.dataElement.code == 'WorkingStatus' )
                    {
                        var promiseTei = TEIService.getTrackedEntityInstance($scope.currentEvent.trackedEntityInstance);
                        promiseTei.then(function (tei) {
                            angular.forEach(tei.attributes, function (att) {
                                if (att.displayName == "Working Status") {
                                    att.value = value;
                                }
                            });
                            TEIService.update(tei, $scope.optionSets, $scope.attributesById).then(function (updateResponse) {
                                if( updateResponse.status == 'SUCCESS')
                                {
                                    ...
                                    $timeout(function() {
                                        $rootScope.$broadcast('dashboardWidgets', {});
                                    }, 100);
            ...
            if (!angular.isUndefined($scope.note) && $scope.note !== "") {
                ...
                if (angular.isUndefined($scope.currentEvent.notes)) {
                    $scope.currentEvent.notes = [
                        {value: $scope.note, storedDate: today, storedBy: storedBy}
                    ];
                }
                else {
    ...
    .controller('EventCreationController',
    function ($scope, $modalInstance, DateUtils, DHIS2EventFactory, DialogService, stagesById, dummyEvent, programId, trackedEntityInstanceId) {
