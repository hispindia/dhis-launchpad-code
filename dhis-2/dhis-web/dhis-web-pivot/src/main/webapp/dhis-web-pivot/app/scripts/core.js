	...
	PT.core.getInstance = function(init) {
		...
		(function() {
			...
			util.array = {
				...
				sort: function(array, direction, key) {
		...
		(function() {
			...
			if (init.legendSets) {
				init.legendSets = util.array.sortObjectsByString(init.legendSets);
	...
	//					[ou1, ou2, ou3, ou4] ]
		...
		//aColIds	= [ abc, bcd, ... ]
					...
					for (var i = 0, allRow; i < aAllItems.length; i++) {
						...
						for (var j = 0, obj = {}; j < aAllItems[i].length; j++) {
							obj = {
								id: aAllItems[i][j],
					...
					for (var i = 0; i < aAllObjects.length; i++) {
						//for (var j = 0, obj, doorCount = 0, oldestObj; j < aAllObjects[i].length; j += aSpan[i]) {
						for (var j = 0, obj, doorCount = 0, oldestObj; j < aAllObjects[i].length; j++) {
							...
							if (doorCount === 0) {
								...
								obj.children = obj.leaf ? 0 : aSpan[i];
					...
					if (aAllObjects.length > 1) {
						for (var i = 1, allRow; i < aAllObjects.length; i++) {
							...
							for (var j = 0, doorCount = 0, span = aSpan[i - 1]; j < allRow.length; j++) {
								allRow[j].parent = aAllObjects[i - 1][j];
								//obj = allRow[j];
					...
					if (aAllObjects.length) {
						...
						var span = aAllObjects.length > 1 ? util.array.sort(Ext.clone(aSpan))[1] : nCols,
							allFloorObjectsLast = aAllObjects[aAllObjects.length - 1];
	...
	//console.log("aAllObjects", aAllObjects);
				...
				getTableHtml = function(xColAxis, xRowAxis, xResponse) {
					...
					getRowHtmlArray = function() {
						...
						// Value objects
						for (var i = 0, valueItemsRow, valueObjectsRow, idValueMap = xResponse.idValueMap; i < rowSize; i++) {
							valueItemsRow = [];
							valueObjectsRow = [];
